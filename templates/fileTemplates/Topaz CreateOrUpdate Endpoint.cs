...
internal sealed class CreateOrUpdate${NAME}Endpoint(Pipeline eventPipeline, ITopazLogger logger)
    : IEndpointDefinition
{
    ...
    [
        "PUT /subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/${PROVIDER_NAMESPACE}/${RESOURCE_TYPE}/{name}"
    ];

    public string[] Permissions => ["${PROVIDER_NAMESPACE}/${RESOURCE_TYPE}/write"];
    ...
    public void GetResponse(HttpContext context, HttpResponseMessage response, GlobalOptions options)
    {
        ...
        var request = JsonSerializer.Deserialize<${REQUEST_TYPE}>(reader.ReadToEnd(), GlobalSettings.JsonOptions);
        if (request == null)
        {
            response.StatusCode = HttpStatusCode.BadRequest;
            return;
