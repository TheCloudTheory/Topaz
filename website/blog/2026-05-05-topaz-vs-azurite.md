...
| Queue ACL | Yes | Yes |
| RA-GRS secondary endpoints | ✅ (v1.4: DNS, stats, read-only enforcement; v1.6: general reads on secondary, without replication lag simulation) | Yes |

As of v1.4, SAS token validation (Account SAS and Service SAS), stored access policy enforcement, and public-access Blob reads have shipped on the data plane. RA-GRS secondary endpoint DNS registration, `GeoReplicationStats` payloads, and read-only enforcement are also live. As of v1.6, general data reads through secondary endpoints are supported as well, without replication lag simulation. See the Update sections at the end of this post for a full summary.
...
The Storage roadmap is publicly tracked in [`BACKLOG.md`](https://github.com/TheCloudTheory/Topaz/blob/main/BACKLOG.md) and mirrored on the [website roadmap](/roadmap). The v1.4, v1.5, and v1.6 items below have shipped. See the [roadmap](/roadmap) for current milestone status.
...
**✅ v1.6: unified data-plane port.** _(shipped)_
Real Azure exposes Blob / Table / Queue / File on port 443 with subdomain-based routing (`{account}.blob.core.windows.net`, etc.). Topaz now serves all Storage sub-services on a single HTTPS port (`8891`) with Host header subdomain routing. Azure CLI and SDK code paths that construct a single URL from the cloud-suffix `storage_endpoint` now work without a port mismatch.
...
**RA-GRS secondary endpoint reads.** Standard GET operations on `{accountName}-secondary.*` endpoints (blob container listing, queue listing, table entity reads) now return data for RA-GRS and RA-GZRS storage accounts. Secondary reads are always perfectly in sync with primary. Replication lag simulation is not yet implemented and is tracked separately on the v1.9 roadmap.

**Unified Storage data-plane port.** Blob, Table, Queue, and File Storage now share a single HTTPS port (`8891`). The router disambiguates sub-services by Host header subdomain (`{account}.blob.*`, `{account}.table.*`, etc.), matching the real Azure URL pattern. This also fixes `az storage` code paths that construct a single URL from the cloud-suffix `storage_endpoint` and previously hit a port mismatch.

---

## Summary

Azurite is a good Storage emulator. Topaz is a Storage emulator that also covers Key Vault, Service Bus, Event Hubs, Container Registry, Managed Identity, RBAC, ARM, and Entra ID, in one binary, with one log stream, one working directory, and one Docker image. The Storage parity is now complete as of v1.6: SAS validation, public-access reads, User Delegation SAS, and RA-GRS secondary reads are all implemented.
...
If you are evaluating local workflows, continue with [Using Topaz CLI](https://topaz.thecloudtheory.com/docs/using-cli/), [Local Azure emulator](https://topaz.thecloudtheory.com/docs/local-azure-emulator/), [Topaz vs. Azurite: full comparison](/docs/comparisons/azurite-alternative/), and [Pricing](https://topaz.thecloudtheory.com/pricing/).
:::
