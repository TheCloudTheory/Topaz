...
Az PowerShell uses the .NET `HttpClient` stack, which on most platforms respects the OS certificate store. If the Topaz certificate is already trusted at the OS level (for example, you ran `topaz-host` and the certificate was accepted), Az PowerShell will work without additional configuration.
...
| Subscription not found after login | No subscription created | Add `--default-subscription` to `topaz-host` |
