...
- Your application relies on scheduled message delivery or message deferral
...
Before migrating, verify that your application does not rely on scheduled messages or message deferral — these are planned for v1.12. Dead-letter queues (including the explicit `DeadLetter()` receiver API, auto-DLQ on max delivery count, and TTL expiry) are fully supported. See the [roadmap](/roadmap) for the full list.
