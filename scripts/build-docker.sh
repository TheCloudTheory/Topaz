...
rm -rf ./publish
dotnet publish ./Topaz.CLI/Topaz.CLI.csproj -c Release -r $DOTNET_RID -o ./publish
docker build -f ./Topaz.CLI/Dockerfile -t topaz/cli --platform linux/$PLATFORM --progress=plain --no-cache --build-arg TARGETPLATFORM=linux/$PLATFORM .
