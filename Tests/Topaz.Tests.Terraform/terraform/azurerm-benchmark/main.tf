# Benchmark variant of azurerm-combined/main.tf.
# Identical resource set, but Key Vaults use access policies instead of RBAC
# so the benchmark caller (az login identity) can manage keys and certificates
# without needing separate role assignments.
# Do NOT use this file in the Topaz test suite.
...
resource "azurerm_key_vault" "kv_certs" {
  ...
  access_policy {
    ...
    key_permissions         = ["Backup", "Create", "Decrypt", "Delete", "Encrypt", "Get", "GetRotationPolicy", "Import", "List", "Purge", "Recover", "Restore", "SetRotationPolicy", "Sign", "UnwrapKey", "Update", "Verify", "WrapKey"]
