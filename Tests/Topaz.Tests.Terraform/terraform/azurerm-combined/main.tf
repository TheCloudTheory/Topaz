...
resource "azurerm_resource_group" "avset_rg" {
  name     = "tf-rm-avset-rg"
  location = "westeurope"
}
...
resource "azurerm_availability_set" "avset" {
  name                         = "tf-rm-avset"
  resource_group_name          = azurerm_resource_group.avset_rg.name
  location                     = azurerm_resource_group.avset_rg.location
  platform_fault_domain_count  = 2
  platform_update_domain_count = 5
}

output "avset_name"                         { value = azurerm_availability_set.avset.name }
