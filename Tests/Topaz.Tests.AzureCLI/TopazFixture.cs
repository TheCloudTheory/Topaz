...
public class TopazFixture
{
    private static readonly string AzureCliContainerImage =
        Environment.GetEnvironmentVariable("AZURE_CLI_CONTAINER_IMAGE") ?? "mcr.microsoft.com/azure-cli:2.88.0";
...
""";
    ...
    public async Task OneTimeSetUp()
    {
        ...
        _containerAzureCli = new ContainerBuilder()
            ...
            .WithResourceMapping(Encoding.UTF8.GetBytes(await File.ReadAllTextAsync(Path.Combine(templatesPath, "sql-server-deployment.json"))), "/templates/sql-server-deployment.json")
            ...
            .WithResourceMapping(Encoding.UTF8.GetBytes(await File.ReadAllTextAsync(Path.Combine(templatesPath, "subscription-scope-deployment.json"))), "/templates/subscription-scope-deployment.json")
            ...
            .WithExtraHost("topazstortblentdel01.table.storage.topaz.local.dev", _containerTopaz.IpAddress)
            .WithExtraHost("topazstortblshow01.table.storage.topaz.local.dev", _containerTopaz.IpAddress)
