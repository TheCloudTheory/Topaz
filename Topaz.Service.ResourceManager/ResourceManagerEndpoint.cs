    ...
    TemplateDeploymentOrchestrator deploymentOrchestrator) : IEndpointDefinition
{
    ...
    public void GetResponse(HttpContext context, HttpResponseMessage response, GlobalOptions options)
    {
        try
        {
            ...
            var resourceGroupOperation =
                _resourceGroupControlPlane.Get(subscriptionIdentifier, resourceGroupIdentifier);
            ...
            switch (context.Request.Method)
            {
                case "PUT":
                    ...
                    {
                        HandleCreateOrUpdateDeployment(response, subscriptionIdentifier,
                            resourceGroupOperation.Resource!, deploymentName, context.Request.Body);
                    }
                ...
                case "DELETE":
                    if (!string.IsNullOrWhiteSpace(deploymentName))
                    {
                        HandleDeleteDeployment(response, subscriptionIdentifier, resourceGroupIdentifier,
                            deploymentName);
                    }
    ...
    private static bool CanHandleRequestBasedOnAvailableData(string[] segments,
        ControlPlaneOperationResult<ResourceGroupResource> resourceGroupOperation)
    {
        return segments.Length != 5 && (resourceGroupOperation.Result == OperationResult.NotFound ||
                                        resourceGroupOperation.Resource == null);
    }

    private void HandleGetResourceProviderData(HttpResponseMessage response,
        SubscriptionIdentifier subscriptionIdentifier, string providerName)
    {
    ...
    private void HandleValidateDeployment(HttpResponseMessage response, SubscriptionIdentifier subscriptionIdentifier,
        ResourceGroupIdentifier resourceGroupIdentifier, string deploymentName, Stream input)
    {
    ...
    private void HandleGetDeployment(HttpResponseMessage response, SubscriptionIdentifier subscriptionIdentifier,
        ResourceGroupIdentifier resourceGroupIdentifier, string deploymentName)
    {
        var deploymentOperation =
            _controlPlane.GetDeployment(subscriptionIdentifier, resourceGroupIdentifier, deploymentName);
