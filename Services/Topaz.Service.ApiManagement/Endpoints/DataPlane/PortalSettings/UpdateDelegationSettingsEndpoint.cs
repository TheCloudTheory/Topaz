...
internal sealed class UpdateDelegationSettingsEndpoint(Pipeline eventPipeline, ITopazLogger logger)
    : IEndpointDefinition
{
    ...
    public void GetResponse(HttpContext context, HttpResponseMessage response, GlobalOptions options)
    {
        ...
        var request =
            JsonSerializer.Deserialize<CreateOrUpdatePortalDelegationSettingsRequest>(reader.ReadToEnd(), GlobalSettings.JsonOptions);
        if (request == null)
        {
            response.StatusCode = HttpStatusCode.BadRequest;
            return;
        ...
        var result = _controlPlane.UpdateDelegationSettings(sub, rg, name, request, ifMatch);
