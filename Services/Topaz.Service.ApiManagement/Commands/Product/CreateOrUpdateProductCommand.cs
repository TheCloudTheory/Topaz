...
[UsedImplicitly]
[CommandDefinition("apim product create", "api-management", "Creates or updates a product in an Azure API Management service.")]
[CommandExample("Creates a new product in an API Management service",
...
internal sealed class CreateOrUpdateProductCommand(HttpClient httpClient, DefaultsProvider provider)
    : TopazHttpCommand<CreateOrUpdateProductCommand.CreateOrUpdateProductCommandSettings>(httpClient)
{
    protected override async Task<int> ExecuteAsync(CommandContext context, CreateOrUpdateProductCommandSettings settings, CancellationToken cancellationToken)
    {
    ...
    protected override ValidationResult Validate(CommandContext context, CreateOrUpdateProductCommandSettings settings)
    {
        ...
        if (string.IsNullOrEmpty(settings.DisplayName))
            return ValidationResult.Error("Display name can't be null.");
    ...
    [UsedImplicitly]
    public sealed class CreateOrUpdateProductCommandSettings : CommandSettings
    {
        ...
        [CommandOptionDefinition("(Required) display name of the product")]
        [CommandOption("--display-name")]
        public string? DisplayName { get; set; }

        [CommandOptionDefinition("(Optional) description of the product")]
