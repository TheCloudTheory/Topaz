...
[UsedImplicitly]
[CommandDefinition("apim product remove-api", "api-management", "Removes an API from a product in an Azure API Management service.")]
[CommandExample("Removes an API from a product",
    "topaz apim product remove-api --subscription-id 36a28ebb-9370-46d8-981c-84efe02048ae \\\n    --service-name \"my-apim\" \\\n    --product-id \"my-product\" \\\n    --api-id \"my-api\" \\\n    --resource-group \"rg-local\"")]
internal sealed class DeleteProductApiCommand(HttpClient httpClient, DefaultsProvider provider)
    : TopazHttpCommand<DeleteProductApiCommand.DeleteProductApiCommandSettings>(httpClient)
{
    protected override async Task<int> ExecuteAsync(CommandContext context, DeleteProductApiCommandSettings settings, CancellationToken cancellationToken)
    {
        ...
        AnsiConsole.WriteLine($"API '{settings.ApiId}' removed from product '{settings.ProductId}'.");
        return 0;
    }

    protected override ValidationResult Validate(CommandContext context, DeleteProductApiCommandSettings settings)
    {
    ...
    [UsedImplicitly]
    public sealed class DeleteProductApiCommandSettings : CommandSettings
    {
