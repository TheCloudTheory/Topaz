...
internal sealed class Resp2ProtocolHandler(RedisServiceControlPlane controlPlane, ITopazLogger logger)
{
    ...
    private byte[] HandleDecr(List<byte[]> parameters)
    {
        ...
        var result = _dataPlane.Decr(key, _cache!);
    ...
    {
        logger.LogDebug(nameof(Resp2ProtocolHandler), nameof(HandleRPop), $"Received SMEMBERS command.");

        var key = parameters[0];
        var result = _dataPlane.SMembers(key, _cache!);
