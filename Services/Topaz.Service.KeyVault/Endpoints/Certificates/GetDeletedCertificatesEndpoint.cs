...
internal sealed class GetDeletedCertificatesEndpoint(Pipeline eventPipeline, ITopazLogger logger)
    : KeyVaultDataPlaneEndpointBase(eventPipeline, logger), IEndpointDefinition
{
    ...
    public string[] Endpoints => ["GET /deletedcertificates", "GET /deletedcertificates/"];
    ...
    public void GetResponse(HttpContext context, HttpResponseMessage response, GlobalOptions options)
    {
        try
        {
            ...
            var operation = _dataPlane.GetDeletedCertificates(
                vault.GetSubscription(), vault.GetResourceGroup(), vault.Name!);

            response.CreateJsonContentResponse(
                GetDeletedCertificatesResponse.FromRecords(operation.Resource!, vault.Name!));
        }
        catch (Exception ex)
        {
            Logger.LogError(ex);
