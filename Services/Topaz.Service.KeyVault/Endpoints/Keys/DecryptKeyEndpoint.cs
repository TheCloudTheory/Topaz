...
internal sealed class DecryptKeyEndpoint(Pipeline eventPipeline, ITopazLogger logger) : KeyVaultDataPlaneEndpointBase(eventPipeline, logger), IEndpointDefinition
{
    ...
    public string[] Endpoints => ["POST /keys/{keyName}/{keyVersion}/decrypt"];

    public string[] Permissions => ["Microsoft.KeyVault/vaults/keys/decrypt/action"];

    public (ushort[] Ports, Protocol Protocol) PortsAndProtocol =>
        ([GlobalSettings.DefaultKeyVaultPort, GlobalSettings.HttpsPort], Protocol.Https);

    protected override string? AccessPolicyPermission => "decrypt";
    ...
    public void GetResponse(HttpContext context, HttpResponseMessage response, GlobalOptions options)
    {
        try
        {
            ...
            var operation = _dataPlane.DecryptKey(subscriptionIdentifier, resourceGroupIdentifier,
