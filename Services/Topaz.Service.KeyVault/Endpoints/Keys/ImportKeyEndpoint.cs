...
internal sealed class ImportKeyEndpoint(Pipeline eventPipeline, ITopazLogger logger) : KeyVaultDataPlaneEndpointBase(eventPipeline, logger), IEndpointDefinition
{
    ...
    protected override string? AccessPolicyPermission => "import";
    ...
    public void GetResponse(HttpContext context, HttpResponseMessage response, GlobalOptions options)
    {
        try
        {
            ...
            var operation = _dataPlane.ImportKey(context.Request.Body,
                subscriptionIdentifier, resourceGroupIdentifier, vaultName!, keyName!);

            if (operation.Result == OperationResult.Failed)
            {
                response.CreateErrorResponse("BadRequest", operation.Reason ?? "Bad request.", HttpStatusCode.BadRequest);
                return;
            }
