...
internal sealed class ReleaseKeyEndpoint(Pipeline eventPipeline, ITopazLogger logger) : KeyVaultDataPlaneEndpointBase(eventPipeline, logger), IEndpointDefinition
{
    ...
    public string[] Endpoints => ["POST /keys/{keyName}/{keyVersion}/release", "POST /keys/{keyName}/release"];

    public string[] Permissions => ["Microsoft.KeyVault/vaults/keys/release/action"];

    public (ushort[] Ports, Protocol Protocol) PortsAndProtocol =>
        ([GlobalSettings.DefaultKeyVaultPort, GlobalSettings.HttpsPort], Protocol.Https);

    protected override string? AccessPolicyPermission => "release";
    ...
    public void GetResponse(HttpContext context, HttpResponseMessage response, GlobalOptions options)
    {
        try
        {
            ...
            // Support both /keys/{name}/{version}/release and /keys/{name}/release
            ...
            var request = JsonSerializer.Deserialize<ReleaseKeyRequest>(rawBody, GlobalSettings.JsonOptions);
            if (request == null)
            {
            ...
            var operation = _dataPlane.ReleaseKey(subscriptionIdentifier, resourceGroupIdentifier,
