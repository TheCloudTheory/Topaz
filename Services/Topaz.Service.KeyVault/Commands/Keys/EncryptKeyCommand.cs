...
[UsedImplicitly]
[CommandDefinition("keyvault key encrypt", "key-vault", "Encrypts plaintext using a Key Vault key (RSA keys only: RSA1_5, RSA-OAEP, RSA-OAEP-256).")]
[CommandExample("Encrypt with RSA-OAEP-256",
    "topaz keyvault key encrypt --vault-name \"kvlocal\" --name \"my-key\" --version \"abc123\" --algorithm \"RSA-OAEP-256\" --value \"aGVsbG8=\" --resource-group \"rg-local\" --subscription-id \"36a28ebb-9370-46d8-981c-84efe02048ae\"")]
public class EncryptKeyCommand(ITopazLogger logger) : Command<EncryptKeyCommand.EncryptKeyCommandSettings>
{
    public override int Execute(CommandContext context, EncryptKeyCommandSettings settings)
    {
        logger.LogInformation($"Executing {nameof(EncryptKeyCommand)}.{nameof(Execute)}.");
        ...
        var operation = dataPlane.EncryptKey(subscriptionIdentifier, resourceGroupIdentifier,
    ...
    [UsedImplicitly]
    public sealed class EncryptKeyCommandSettings : CommandSettings
    {
        ...
        [CommandOptionDefinition("(Required) Encryption algorithm. Supported: RSA1_5, RSA-OAEP, RSA-OAEP-256.", required: true)]
        [CommandOption("-a|--algorithm")]
        public string? Algorithm { get; set; }

        [CommandOptionDefinition("(Required) Base64url-encoded plaintext to encrypt.", required: true)]
        [CommandOption("--value")]
