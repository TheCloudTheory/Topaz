...
internal sealed class ListPublicIpAddressesBySubscriptionEndpoint(Pipeline eventPipeline, ITopazLogger logger) : IEndpointDefinition
{
    ...
    {
        logger.LogDebug(nameof(ListPublicIpAddressesBySubscriptionEndpoint), nameof(GetResponse), "Executing {0}.", nameof(GetResponse));

        try
        {
            var subscriptionIdentifier = SubscriptionIdentifier.From(context.Request.Path.Value!.ExtractValueFromPath(2));

            var pips = _controlPlane.ListBySubscription(subscriptionIdentifier);
