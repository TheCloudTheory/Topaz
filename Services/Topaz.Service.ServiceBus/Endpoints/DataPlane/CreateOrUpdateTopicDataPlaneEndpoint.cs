...
internal sealed class CreateOrUpdateTopicDataPlaneEndpoint(Pipeline eventPipeline, ITopazLogger logger) : IEndpointDefinition
{
    ...
    public string[] Endpoints => ["PUT /{entity}/{messageType}"];
    ...
    public void GetResponse(HttpContext context, HttpResponseMessage response, GlobalOptions options)
    {
        ...
        if (serializer.Deserialize(new StringReader(content)) is not CreateOrUpdateServiceBusTopicAtomRequest atomRequest)
        {
            response.StatusCode = HttpStatusCode.InternalServerError;
            return;
        }

        var operation = _controlPlane.CreateOrUpdateTopic(subscriptionId!, resourceGroupId!, namespaceName, topicName, CreateOrUpdateServiceBusTopicRequest.From(atomRequest));
