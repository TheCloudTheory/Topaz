...
internal sealed class DeleteEntityDataPlaneEndpoint(Pipeline eventPipeline, ITopazLogger logger) : IEndpointDefinition
{
    ...
    public string[] Endpoints => ["DELETE /{entity}"];
    public string[] Permissions => ["Microsoft.ServiceBus/namespaces/queues/delete", "Microsoft.ServiceBus/namespaces/topics/delete"];
    ...
    public void GetResponse(HttpContext context, HttpResponseMessage response, GlobalOptions options)
    {
        ...
        switch (entityType)
        {
            case ServiceBusEntityType.Queue:
                var queueOperation = _controlPlane.DeleteQueue(subscriptionId!, resourceGroupId!, namespaceName, entityName!);
                if (queueOperation.Result == OperationResult.NotFound)
                {
                    response.StatusCode = HttpStatusCode.NotFound;
                    return;
                }
            ...
            case ServiceBusEntityType.Topic:
                var topicOperation = _controlPlane.DeleteTopic(subscriptionId!, resourceGroupId!, namespaceName, entityName!);
                if (topicOperation.Result == OperationResult.NotFound)
                {
                    response.StatusCode = HttpStatusCode.NotFound;
                    return;
                }
