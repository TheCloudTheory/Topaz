...
internal sealed class CreateOrUpdateSqlContainerEndpoint(Pipeline eventPipeline, ITopazLogger logger)
    : IEndpointDefinition
{
    ...
    public string[] Permissions => ["Microsoft.DocumentDB/databaseAccounts/sqlDatabases/containers/write"];
    ...
    public void GetResponse(HttpContext context, HttpResponseMessage response, GlobalOptions options)
    {
        ...
        if (string.IsNullOrWhiteSpace(accountName) || string.IsNullOrWhiteSpace(databaseName) || string.IsNullOrWhiteSpace(containerName))
        {
        ...
        logger.LogDebug(nameof(CreateOrUpdateSqlContainerEndpoint), nameof(GetResponse),
            "Processing payload: {0}", content);

        var request = JsonSerializer.Deserialize<CreateOrUpdateSqlContainerRequest>(content, GlobalSettings.JsonOptions);
        if (request == null)
        {
        ...
        var operation = _controlPlane.CreateOrUpdateSqlContainer(
