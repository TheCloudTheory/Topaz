...
internal sealed class ListConnectionStringsDatabaseAccountEndpoint(Pipeline eventPipeline, ITopazLogger logger)
    : IEndpointDefinition
{
    ...
    public string[] Permissions => ["Microsoft.DocumentDB/databaseAccounts/listConnectionStrings/action"];
    ...
    {
        logger.LogDebug(nameof(ListConnectionStringsDatabaseAccountEndpoint), nameof(GetResponse),
        ...
        try
        {
            ...
            var operation = _controlPlane.GetConnectionStrings(subscriptionIdentifier, resourceGroupIdentifier, accountName!);
