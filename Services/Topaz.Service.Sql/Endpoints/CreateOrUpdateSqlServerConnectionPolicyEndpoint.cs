...
internal sealed class CreateOrUpdateSqlServerConnectionPolicyEndpoint(Pipeline eventPipeline, ITopazLogger logger)
    : IEndpointDefinition
{
    ...
    public string[] Permissions => ["Microsoft.Sql/servers/connectionPolicies/write"];
    ...
    {
        logger.LogDebug(nameof(CreateOrUpdateSqlServerConnectionPolicyEndpoint), nameof(GetResponse),
        ...
        try
        {
            ...
            var policyName = context.Request.Path.Value.ExtractValueFromPath(10);

            if (string.IsNullOrWhiteSpace(serverName) || string.IsNullOrWhiteSpace(policyName))
            {
            ...
            var request = JsonSerializer.Deserialize<CreateOrUpdateSqlServerConnectionPolicyRequest>(
                content, GlobalSettings.JsonOptions);

            var properties = request == null
                ? SqlServerConnectionPolicySubresourceProperties.Default()
                : SqlServerConnectionPolicySubresourceProperties.FromRequest(request);

            var result = _controlPlane.CreateOrUpdateConnectionPolicy(
