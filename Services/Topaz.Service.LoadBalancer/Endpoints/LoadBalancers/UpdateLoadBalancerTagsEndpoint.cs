...
internal sealed class UpdateLoadBalancerTagsEndpoint(Pipeline eventPipeline, ITopazLogger logger) : IEndpointDefinition
{
    ...
    {
        logger.LogDebug(nameof(UpdateLoadBalancerTagsEndpoint), nameof(GetResponse), "Executing {0}.", nameof(GetResponse));
        ...
        try
        {
            ...
            var request = JsonSerializer.Deserialize<UpdateLoadBalancerTagsRequest>(content, GlobalSettings.JsonOptions);
            ...
            var result = _controlPlane.UpdateTags(subscriptionIdentifier, resourceGroupIdentifier, loadBalancerName!, request);
