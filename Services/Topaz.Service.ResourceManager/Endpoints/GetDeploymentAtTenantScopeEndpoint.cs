...
public sealed class GetDeploymentAtTenantScopeEndpoint(
    ITopazLogger logger,
    TemplateDeploymentOrchestrator orchestrator) : IEndpointDefinition
{
    ...
    public void GetResponse(HttpContext context, HttpResponseMessage response, GlobalOptions options)
    {
        ...
        logger.LogDebug(nameof(GetDeploymentAtTenantScopeEndpoint), nameof(GetResponse),
            "Getting tenant-scope deployment: `{0}`", deploymentName);

        var result = _controlPlane.Get(deploymentName!);
