...
internal sealed class UpdateVirtualMachineEndpoint(Pipeline eventPipeline, ITopazLogger logger) : IEndpointDefinition
{
    ...
    [
        "PATCH /subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Compute/virtualMachines/{virtualMachineName}"
    ];
    ...
    {
        logger.LogDebug(nameof(UpdateVirtualMachineEndpoint), nameof(GetResponse), "Executing {0}.", nameof(GetResponse));
        ...
        try
        {
            ...
            logger.LogDebug(nameof(UpdateVirtualMachineEndpoint), nameof(GetResponse),
                "Processing payload: {0}", content);

            var request = JsonSerializer.Deserialize<UpdateVirtualMachineRequest>(content, GlobalSettings.JsonOptions);
            if (request == null)
            {
            ...
            var result = _controlPlane.Update(subscriptionIdentifier, resourceGroupIdentifier, virtualMachineName, request);
