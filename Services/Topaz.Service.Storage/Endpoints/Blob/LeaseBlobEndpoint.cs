...
internal sealed class LeaseBlobEndpoint(Pipeline eventPipeline, ITopazLogger logger)
    : BlobDataPlaneEndpointBase(eventPipeline, logger), IEndpointDefinition
{
    ...
    public string[] Endpoints => ["PUT /{containerName}/...?comp=lease"];
    ...
    public void GetResponse(HttpContext context, HttpResponseMessage response, GlobalOptions options)
    {
        ...
        try
        {
            ...
            Logger.LogDebug(nameof(LeaseBlobEndpoint), nameof(GetResponse),
                "Lease action '{0}' on blob: {1}", leaseAction, blobPath);

            var op = _dataPlane.LeaseBlob(
            ...
            if (op.Resource == null)
            {
                response.CreateBlobErrorResponse(
                    ...
                    GetLeaseErrorMessage(op.Result, leaseAction, blobPath, currentLeaseId),
                    response.StatusCode);
                return;
            }
    ...
    private static string GetLeaseErrorCode(OperationResult result, string leaseAction, string? currentLeaseId)
    {
        ...
        {
            OperationResult.NotFound => "BlobNotFound",
            OperationResult.PreconditionFailed => "LeaseIdMismatchWithBlobOperation",
            ...
            OperationResult.Conflict when RequiresLeaseId(leaseAction) && string.IsNullOrWhiteSpace(currentLeaseId) =>
                "LeaseIdMissing",
            OperationResult.Conflict => "LeaseNotPresentWithBlobOperation",
            _ => "OperationNotAllowed"
        };
    }

    private static string GetLeaseErrorMessage(OperationResult result, string leaseAction, string blobPath,
        string? currentLeaseId)
    {
        return result switch
        {
            OperationResult.NotFound => $"The specified blob '{blobPath}' was not found.",
            ...
            OperationResult.Conflict when string.Equals(leaseAction, "acquire", StringComparison.OrdinalIgnoreCase) =>
                "There is already an active lease on the blob.",
            OperationResult.Conflict when RequiresLeaseId(leaseAction) && string.IsNullOrWhiteSpace(currentLeaseId) =>
                "A lease ID must be specified for this lease operation.",
            OperationResult.Conflict => "There is no active lease on the blob.",
