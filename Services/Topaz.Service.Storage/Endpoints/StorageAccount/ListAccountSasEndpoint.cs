...
internal sealed class ListAccountSasEndpoint(ITopazLogger logger) : IEndpointDefinition
{
    private readonly AzureStorageControlPlane _controlPlane = new(new ResourceProvider(logger), logger);

    public string[] Endpoints =>
    [
        "POST /subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Storage/storageAccounts/{storageAccountName}/ListAccountSas"
    ];

    public string[] Permissions => ["Microsoft.Storage/storageAccounts/listAccountSas/action"];
    ...
    public void GetResponse(HttpContext context, HttpResponseMessage response, GlobalOptions options)
    {
        try
        {
            ...
            var request = JsonSerializer.Deserialize<ListAccountSasRequest>(content, GlobalSettings.JsonOptions);
            ...
            if (string.IsNullOrWhiteSpace(request.SignedServices) ||
                string.IsNullOrWhiteSpace(request.SignedResourceTypes) ||
            ...
            var result = _controlPlane.ListAccountSas(subscriptionIdentifier, resourceGroupIdentifier, storageAccountName, request);
