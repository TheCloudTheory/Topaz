...
internal sealed class SetQueueAclEndpoint(Pipeline eventPipeline, ITopazLogger logger)
    : QueueDataPlaneEndpointBase(eventPipeline, logger), IEndpointDefinition
{
    ...
    public void GetResponse(HttpContext context, HttpResponseMessage response, GlobalOptions options)
    {
        ...
        try
        {
            ...
            Logger.LogDebug(nameof(SetQueueAclEndpoint), nameof(GetResponse),
                "Setting ACL for queue: {0}.", queueName);

            var result = _dataPlane.SetQueueAcl(subscriptionIdentifier, resourceGroupIdentifier,
                storageAccount.Name, queueName, context.Request.Body);
