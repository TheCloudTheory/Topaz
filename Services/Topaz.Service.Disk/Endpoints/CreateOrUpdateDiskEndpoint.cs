...
internal sealed class CreateOrUpdateDiskEndpoint(Pipeline eventPipeline, ITopazLogger logger)
    : IEndpointDefinition
{
    ...
    {
        logger.LogDebug(nameof(CreateOrUpdateDiskEndpoint), nameof(GetResponse),
        ...
        try
        {
            ...
            var request = JsonSerializer.Deserialize<CreateOrUpdateDiskRequest>(content, GlobalSettings.JsonOptions);
            if (request == null)
            {
            ...
            var result = _controlPlane.CreateOrUpdate(
                subscriptionIdentifier, resourceGroupIdentifier, diskName, request);
