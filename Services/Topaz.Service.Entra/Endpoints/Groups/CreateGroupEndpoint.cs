...
internal sealed class CreateGroupEndpoint(ITopazLogger logger) : IEndpointDefinition
{
    ...
    {
        logger.LogDebug(nameof(CreateGroupEndpoint), nameof(GetResponse), "Creating a group.");

        using var reader = new StreamReader(context.Request.Body);
        var content = reader.ReadToEnd();
        var request = JsonSerializer.Deserialize<CreateGroupRequest>(content, GlobalSettings.JsonOptions);
        ...
        if (operation.Result != OperationResult.Created || operation.Resource == null)
        {
            response.CreateErrorResponse(HttpResponseMessageExtensions.InternalErrorCode,
                "Unknown error when performing CreateGroup operation.");
            return;
        }
